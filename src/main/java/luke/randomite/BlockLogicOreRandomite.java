...
public class BlockLogicOreRandomite extends BlockLogic {
    ...
    public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
        if (IS_DEEP_LOADED) {
            int roll = world.rand.nextInt(200);
            ...
            switch (dropCause) {
                ...
                case EXPLOSION:
                case PROPER_TOOL:
                    if (roll < 60) return new ItemStack[]{new ItemStack(Items.EGG_CHICKEN)};

                    if (roll < 100) return new ItemStack[]{new ItemStack(Items.COAL)};

                    if (roll < 120) return new ItemStack[]{new ItemStack(Items.ORE_RAW_IRON)};
                    if (roll < 140) return new ItemStack[]{new ItemStack(DEEPItems.rawLead)};

                    if (roll < 150) return new ItemStack[]{new ItemStack(Items.ORE_RAW_GOLD)};
                    if (roll < 160) return new ItemStack[]{new ItemStack(DEEPItems.rawSilver)};

                    if (roll < 168) return new ItemStack[]{new ItemStack(Items.DYE, 4 + world.rand.nextInt(5), 4)};
                    if (roll < 176) return new ItemStack[]{new ItemStack(Items.DUST_REDSTONE, 1 + world.rand.nextInt(4))};
                    if (roll < 184) return new ItemStack[]{new ItemStack(DEEPItems.uranium)};

                    if (roll < 190) return new ItemStack[]{new ItemStack(Items.DIAMOND)};
                    if (roll < 196) return new ItemStack[]{new ItemStack(DEEPItems.amethyst)};

                    if (roll < 198) return new ItemStack[]{new ItemStack(Items.INGOT_STEEL_CRUDE)};
                    return new ItemStack[]{new ItemStack(DEEPItems.bismuth)};
